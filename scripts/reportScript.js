function addBoxEntry(boxContainerId, text) {
	...
	const newBoxEntry = document.createElement('li');
	newBoxEntry.className = 'list-item';
	...
	setInterval(function () {
		...
		fetch(url)
			...
			.then((data) => {
				...
				try {
					percentComplete =
						jsonData.progress.regex.percentageCompleted;
				...
				try {
					if (completed == 'active') {
						dynamicAnalysisStatus =
							jsonData.progress.networkAnalysis.status;
				...
				if (completed == 'waiting') {
					document.getElementById('quePositionElement').innerHTML =
						'Position in que: #' + position;
				} else if (!(completed == 'completed')) {
					document.getElementById('staticAnalysisStatus').innerHTML =
						'Static analysis status: ' + percentComplete;
					document.getElementById('dynamicAnalysisStatus').innerHTML =
						'Dynamic analysis status: ' + dynamicAnalysisStatus;
				}
			})
...
if (completed == 'waiting') {
	...
	if (maliciousFlagsCount == 0) {
		addBoxEntry(
			'staticMaliciousTemplate',
			'No malicious flags were identified'
		);
	}

	if (suspiciousFlagsCount == 0) {
		addBoxEntry(
			'staticSuspiciousTemplate',
			'No suspicious flags were identified'
		);
	}
	...
	try {
		if (
			scanReport.returnvalue.networkAnalysis.generalResults
				.maliciousConnections.length === 0
		) {
			addBoxEntry(
				'dynamicMaliciousTemplate',
				'No malicious connections were made'
			);
		} else {
			scanReport.returnvalue.networkAnalysis.generalResults.maliciousConnections.forEach(
				function (flag) {
		...
		addBoxEntry(
			'dynamicMaliciousTemplate',
			scanReport.returnvalue.networkAnalysis.errorData.messages[0]
		);
	}
	...
	try {
		if (
			...
			addBoxEntry(
				'dynamicUnknownTemplate',
				'No malicious connections were made'
			);
			suspiciousFlagsCount += 1;
		} else {
			scanReport.returnvalue.networkAnalysis.generalResults.unknownConnections.forEach(
				function (flag) {
	...
	if (downloadCount > 10000) {
		...
		noticeSpan.textContent =
			"This mod was found on modrinth with over 10k downloads, it's most likley safe";
